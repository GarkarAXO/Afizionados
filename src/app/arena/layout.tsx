      ...
      <div className={`fixed right-0 top-0 h-screen w-full max-w-md bg-white dark:bg-[#0a0a0a] border-l border-gray-200 dark:border-white/10 z-[101] shadow-2xl transition-transform duration-500 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        {cart.length > 0 && (
          <div className="p-8 border-t border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-[#050505] space-y-6 mb-16 sm:mb-0">
            ...
            <button className="w-full py-5 bg-[#d4af35] text-[#201d13] font-black rounded-2xl uppercase text-xs tracking-[0.2em] hover:brightness-110 shadow-xl">
              Finalizar
            </button>
          </div>
        )}
...
function Navbar() {
  ...
  return (
    <>
      <nav className="h-20 border-b border-gray-200 dark:border-white/5 sticky top-0 z-50 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-xl px-4 sm:px-8 flex items-center justify-between transition-colors">
        <Link href="/arena" className="flex items-center gap-3">
          <img src="/img/afizionadosW.png" alt="Logo" className="h-10 sm:h-12 w-auto object-contain dark:hidden" />
          <img src="/img/afizionadosB.png" alt="Logo" className="h-10 sm:h-12 w-auto object-contain hidden dark:block" />
        </Link>

        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link key={link.href} href={link.href} className={`text-[10px] font-black uppercase tracking-[0.2em] transition-all pb-1 border-b-2 ${pathname === link.href ? 'text-[#d4af35] border-[#d4af35]' : 'text-gray-400 hover:text-gray-900 dark:hover:text-white border-transparent'}`}>
              {link.name}
            </Link>
          ))}
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-6">
          <button onClick={toggleTheme} className="p-2 text-gray-400 hover:text-[#d4af35] transition-all" title="Cambiar Tema">
            <span className="material-symbols-outlined text-xl sm:text-2xl">
              {theme === 'light' ? 'dark_mode' : 'light_mode'}
            </span>
          </button>
          <button onClick={() => setIsCartOpen(true)} className="text-gray-400 hover:text-[#d4af35] transition-colors relative group">
          ...
          <Link href="/auth/login" className="px-4 py-2.5 sm:px-6 bg-[#d4af35] text-[#201d13] font-black rounded-lg text-[9px] sm:text-[10px] uppercase tracking-widest hover:brightness-110 shadow-lg">
            Ingresar
          </Link>
        </div>
      </nav>
        ...
        <footer className="bg-gray-50 dark:bg-[#050505] border-t border-gray-200 dark:border-white/5 py-16 px-8 mt-20 mb-16 md:mb-0 text-center md:text-left transition-colors">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between gap-12">
            <div className="space-y-6 max-w-sm mx-auto md:mx-0">
              ...
              <p className="text-gray-500 text-sm leading-relaxed">La plataforma definitiva para coleccionistas reales.</p>
            </div>
